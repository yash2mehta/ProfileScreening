...
def pdfs_to_cleaned_and_extracted_excel(resume_folder, job_description_file, skills_file, final_excel_path, model=model):
    ...
    for filename in os.listdir(resume_folder):
        ...
        if filename.endswith(".pdf"):
            ...
            # Improve upoan the extract information
            prompt = """
You are provided with the following extracted information from a resume:

Info: {extracted_data}

Your task:
- Keep the original key-value structure intact.
- Refine and restructure the values to make them clear, concise, and accurate.
- For "Name", ensure that only one structured name is provided. Filter out multiple names or unnecessary text.
- For "Location" and "Phone Number", separate them correctly if they are combined or misformatted. Ensure that the phone number is not mixed with the location.
- The "Phone" field must contain only a valid phone number in the correct format, and the "Location" field should include only a single location, without combining multiple places or addresses.
- Eliminate any assumptions or inferences such as "(assumed)" or "estimated" and stick to the data explicitly mentioned in the resume.
- Remove any redundant details or unnecessary text while preserving the core details.
- Ensure the values are well-organized, formatted professionally, and free of errors or inconsistencies.
- If "Score" and "Total Experience" are not calculated, ensure they are computed based on the provided data.
- Do not include any new key information; stick strictly to the original format.
...
"""
            ...
            response = chain.invoke({'extracted_data': extracted_data})
            response = response.replace("\"", "")
            print(f"-- Improved Info Calculated: {response}")

            # Extract Name
            name_match = re.search(r'Name:\s*([^\n]+)', response)
            name = name_match.group(1).strip() if name_match else "Not mentioned"
            ...
            phone = phone_match.group(1).strip() if phone_match else "Not mentioned"

            # Extract Github Links
            github_links_match = re.search(r'Github Links:\s*(\[.*\])', response)
            github_links = eval(github_links_match.group(1)) if github_links_match else []

            # Extract LinkedIn Link
            linkedin_match = re.search(r'LinkedIn Links:\s*(https?://[^\n]+)', response)
            linkedin_link = linkedin_match.group(0).strip() if linkedin_match else "Not mentioned"

            # Extract Total Experience
            experience_match = re.search(r'Total Experience:\s*([^\n]+)', response)
            total_experience = experience_match.group(1).strip() if experience_match else "Not mentioned"

            # Extract Fitment Summary
            fitment_summary_match = re.search(r'Fitment Summary:\s*(.*?)(?=\nScore:|$)', response, re.DOTALL)
            fitment_summary = fitment_summary_match.group(1).strip() if fitment_summary_match else "Not mentioned"

            # Extract Score
            score_match = re.search(r'Score:\s*(\d+)', response)
            score = int(score_match.group(1)) if score_match else "Not mentioned"
            ...
            extracted_data = {
                ...
                "Fitment Summary": fitment_summary,
                "Score": score
            }
